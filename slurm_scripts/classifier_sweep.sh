...
for dir in experiments/local_infomax_encoder_*; do
  ...
  for mode in "fc"; do
    ...
    prefix=classifier_${mode}_${dir}_adv_200
    classifier_ckpt=experiments/${prefix}/classifier_${mode}_${dir}_adv_200_checkpoint.pth
    ...
    "python3 classification.py --encoder_stride ${stride} --classifier_ckpt=${classifier_ckpt} --input_layer ${mode} --gpu \
                  --prefix ${prefix} --classifier_adversarial --batch_size=64 --hidden_units=200 --dropout=0.1 --lr=0.0001 --epochs=2000"
  done
done
