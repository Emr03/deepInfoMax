...
mode=fc
stride=2
for mi_estimator in js; do
 dir=experiments/infomax_classifiers/classifier_${mode}_local_infomax_encoder_${mi_estimator}_no_prior
 name=${dir/experiments\/infomax_classifiers\/}
 classifier_ckpt=${dir}/${name}_checkpoint.pth
 encoder_name=${name/classifier_${mode}_}
 encoder_ckpt=experiments/encoders/${encoder_name}/${encoder_name}_checkpoint.pth
